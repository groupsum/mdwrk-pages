...
test("T2: demo runtime preserves SPA delivery, health checks, and structured-data graph output", () => {
  ...
  assert.ok(bundle.includes("/academy/prompt-delivery-studio/"));
  assert.ok(bundle.includes("Submit quiz"));
  assert.ok(bundle.includes("Submit test"));
  assert.ok(bundle.includes("schema-backed learning paths, courses, modules, quizzes, and tests"));
  assert.ok(bundle.includes("/academy/ai-learning-architect/template-delivery/final-test/"));
  assert.equal(bundle.includes("Current page"), false);
});
