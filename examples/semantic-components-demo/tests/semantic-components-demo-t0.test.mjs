...
test("T0: semantic components demo defines the full 58-type showcase as a dedicated example surface", () => {
  ...
  assert.equal(showcaseEntries.length, 58);
  assert.equal(new Set(showcaseEntries.map((entry) => entry.name)).size, 58);
  assert.equal(showcaseEntriesByFamily.length, 7);
  assert.ok(tokenStyles.includes("semantic-about-page.css"));
  assert.ok(tokenStyles.includes("semantic-aggregate-rating.css"));
  assert.ok(tokenStyles.includes("semantic-video-gallery.css"));
