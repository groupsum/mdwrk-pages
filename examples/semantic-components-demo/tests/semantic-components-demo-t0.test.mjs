...
test("T0: semantic components demo is a mode-driven explorer over the full generated surface", () => {
  ...
  assert.ok(clientSemanticSource.includes("import.meta.glob"));
  assert.ok(clientSemanticSource.includes("semanticStyleImporters"));
  assert.ok(clientSemanticSource.includes("loadSemanticArtifactStyle"));
  assert.ok(clientSemanticSource.includes("pages-ui-tokens/src/styles/semantic-*.css"));
